...
if (metricGrid && stats.length) {
  statsObserver.observe(metricGrid);
}
...
if (booking) {
  ...
  const setError = (input, message) => {
    ...
    $('small', label).textContent = message || '';
  };
  ...
  const valid = () => {
    ...
    setError(
      phone,
      phoneValid ? '' : 'Please enter a valid contact number.'
    );
  ...
  $$('[data-booking-open]').forEach((button) =>
  ...
  $$('[data-booking-close]').forEach((button) =>
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const submitButton = $('.booking-action', form);
    ...
    try {
      ...
      $('.success-summary', booking).textContent =
        `${details.name} · ${details.email}${
          details.instagram ? ` · ${details.instagram}` : ''
        }`;
    } catch (error) {
  ...
  $('.done-booking', booking).addEventListener('click', () => {
    form.reset();
    closeBooking();
  });
}